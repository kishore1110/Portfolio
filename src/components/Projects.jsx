...
/* Card — uses React state for overlay visibility so it works on ALL devices:
   - Desktop: mouse enter/leave controls the overlay
   - Mobile (iOS Safari, Android, OnePlus, CMF, etc.): tap toggles the overlay
     A second tap on the links navigates; tapping elsewhere dismisses. */
function ProjectCard({ project, idx }) {
    const [active, setActive] = useState(false);

    const handleCardClick = (e) => {
        // If the click is on a link, let it through without toggling
    ...
    return (
        <motion.div
            ...
            className={`w-[85vw] md:w-[350px] lg:w-[400px] shrink-0 snap-center relative bg-white dark:bg-[#111827] rounded-3xl overflow-hidden border transition-all duration-500 flex flex-col cursor-pointer
        ${active
        ...
        >
            <div className="aspect-[16/10] overflow-hidden relative bg-gray-100 dark:bg-gray-800">
                ...
                {/* Overlay — controlled by React state, works on all devices */}
                <div
            ...
            <div className="p-6 md:p-8 flex-1 flex flex-col bg-gradient-to-b from-transparent to-gray-50/50 dark:to-black/20">
                <h3 className={`text-2xl font-bold mb-3 transition-colors ${active ? 'text-blue-600 dark:text-blue-400' : 'text-gray-900 dark:text-white'}`}>
                    {project.title}
                </h3>
